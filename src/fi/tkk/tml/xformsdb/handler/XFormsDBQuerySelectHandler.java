...
/**
 * Handle the <xformsdb:query> SELECT queries.
 * 
 *
 * @author Markku Laine
 * @version 1.0	 Created on October 15, 2009
 */
public class XFormsDBQuerySelectHandler extends ResponseHandler {
	
	
	
	// PRIVATE STATIC FINAL VARIABLES
	private static final Logger logger	= Logger.getLogger( XFormsDBQuerySelectHandler.class );

	
	// PUBLIC CONSTURCTORS
	public XFormsDBQuerySelectHandler() {
	...
	private XFormsDBDAO getXFormsDBDAO( XFormsDBServlet xformsdbServlet, Document document ) throws HandlerException {
		...
		try {
			...
			throw new HandlerException( ErrorConstants.ERROR_CODE_XFORMSDBQUERYSELECTHANDLER_1, ErrorConstants.ERROR_MESSAGE_XFORMSDBQUERYSELECTHANDLER_1, ex  );
		}
	...
	public String handle( XFormsDBServlet xformsdbServlet, Document document ) throws HandlerException {
		...
		try {
			...
			logger.log( Level.DEBUG, "The SELECT action of the XFormsDB query request has been successfully handled." );
		} catch ( DAOException daoex ) {
			throw new HandlerException( daoex.getCode(), daoex.getMessage(), daoex );
		} catch ( HandlerException hex ) {
			throw hex;
		} catch ( Exception ex ) {
			throw new HandlerException( ErrorConstants.ERROR_CODE_XFORMSDBQUERYSELECTHANDLER_2, ErrorConstants.ERROR_MESSAGE_XFORMSDBQUERYSELECTHANDLER_2, ex );
		}
