...
/**
 * Handle the <xformsdb:widgetquery> SELECT4UPDATE queries.
 * 
 *
 * @author Markku Laine
 * @version 1.0	 Created on October 23, 2009
 */
public class XFormsDBWidgetQuerySelect4UpdateHandler extends ResponseHandler {
	
	
	
	// PRIVATE STATIC FINAL VARIABLES
	private static final Logger logger			= Logger.getLogger( XFormsDBWidgetQuerySelect4UpdateHandler.class );

	
	// PUBLIC CONSTURCTORS
	public XFormsDBWidgetQuerySelect4UpdateHandler() {
	...
	private XFormsDBDAO getXFormsDBDAO( XFormsDBServlet xformsdbServlet, HttpSession session, Document document, String encoding ) throws HandlerException {
		...
		try {
			...
			if ( xformsdbWidgetDataSourceNodes == null || ( xformsdbWidgetDataSourceNodes != null && xformsdbWidgetDataSourceNodes.size() == 0 ) ) {
				throw new DAOException( ErrorConstants.ERROR_CODE_DAO_134, ErrorConstants.ERROR_MESSAGE_DAO_134 );
			}
			...
			throw new HandlerException( ErrorConstants.ERROR_CODE_XFORMSDBWIDGETQUERYSELECT4UPDATEHANDLER_1, ErrorConstants.ERROR_MESSAGE_XFORMSDBWIDGETQUERYSELECT4UPDATEHANDLER_1, ex  );
		}
	...
	public String handle( XFormsDBServlet xformsdbServlet, HttpSession session, Document document ) throws HandlerException {
		...
		try {
			...
			// Store the widget query result into XFormsDB
			XFormsDBWidgetQueryManager.addSelect4UpdateXFormsDBWidgetQueryResult( session, document.getRootElement().getAttributeValue( "id" ), xmlString );
			logger.log( Level.DEBUG, "The SELECT4UPDATE action of the XFormsDB widget query request has been successfully handled." );
		} catch ( DAOException daoex ) {
			throw new HandlerException( daoex.getCode(), daoex.getMessage(), daoex );
		} catch ( ManagerException mex ) {
			throw new HandlerException( mex.getCode(), mex.getMessage(), mex );
			...
			throw new HandlerException( ErrorConstants.ERROR_CODE_XFORMSDBWIDGETQUERYSELECT4UPDATEHANDLER_2, ErrorConstants.ERROR_MESSAGE_XFORMSDBWIDGETQUERYSELECT4UPDATEHANDLER_2, ex );
		}
