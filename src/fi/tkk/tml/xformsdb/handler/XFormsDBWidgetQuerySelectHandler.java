...
/**
 * Handle the <xformsdb:widgetquery> SELECT queries.
 * 
 *
 * @author Markku Laine
 * @version 1.0	 Created on October 23, 2009
 */
public class XFormsDBWidgetQuerySelectHandler extends ResponseHandler {
	
	
	
	// PRIVATE STATIC FINAL VARIABLES
	private static final Logger logger	= Logger.getLogger( XFormsDBWidgetQuerySelectHandler.class );

	
	// PUBLIC CONSTURCTORS
	public XFormsDBWidgetQuerySelectHandler() {
	...
	private XFormsDBDAO getXFormsDBDAO( XFormsDBServlet xformsdbServlet, HttpSession session, Document document, String encoding ) throws HandlerException {
		...
		try {
			...
			if ( xformsdbWidgetDataSourceNodes == null || ( xformsdbWidgetDataSourceNodes != null && xformsdbWidgetDataSourceNodes.size() == 0 ) ) {
				throw new DAOException( ErrorConstants.ERROR_CODE_DAO_133, ErrorConstants.ERROR_MESSAGE_DAO_133 );
			}
			...
			throw new HandlerException( ErrorConstants.ERROR_CODE_XFORMSDBWIDGETQUERYSELECTHANDLER_1, ErrorConstants.ERROR_MESSAGE_XFORMSDBWIDGETQUERYSELECTHANDLER_1, ex  );
		}
	...
	public String handle( XFormsDBServlet xformsdbServlet, HttpSession session, Document document ) throws HandlerException {
		...
		try {
			...
			logger.log( Level.DEBUG, "The SELECT action of the XFormsDB widget query request has been successfully handled." );
		} catch ( DAOException daoex ) {
			throw new HandlerException( daoex.getCode(), daoex.getMessage(), daoex );
		} catch ( HandlerException hex ) {
			throw hex;
		} catch ( Exception ex ) {
			throw new HandlerException( ErrorConstants.ERROR_CODE_XFORMSDBWIDGETQUERYSELECTHANDLER_2, ErrorConstants.ERROR_MESSAGE_XFORMSDBWIDGETQUERYSELECTHANDLER_2, ex );
		}
