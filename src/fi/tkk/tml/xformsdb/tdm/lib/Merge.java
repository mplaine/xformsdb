...
//
package fi.tkk.tml.xformsdb.tdm.lib;
  ...
  protected int getOperation( BaseNode bn, MergeList ml ) {
    ...
    private void removeDeletedOrMoved( MergeList mlistA, MergeList mlistB ) {
    	...
    	for ( int i = 0; i < baseParent.getChildCount(); i++ ) {
    		 ...
    		 * DELETE  DELETE  OK
    		 ***************************************************************/
    		if ( ( op1 == NOP && op2 == NOP ) || ( op1 == NOP && op2 == MOVE_I ) || ( op1 == MOVE_I && op2 == MOVE_I ) || ( op1 == DELETE && op2 == DELETE ) ) {
    		...
    		if ( op1 == NOP && ( op2 == MOVE_F || op2 == DELETE ) ) {
    			...
    			if ( op2 == DELETE && isDeletiaModified( mlistA.getEntry( ix ).getNode(), mlistA ) ) {
    				// CONFLICTCODE
    				clog.addListConflict( ConflictLog.DELETE, "Modifications in " + "deleted subtree.", bn, mlistA.getEntry( ix ).getNode(), null );
//    				clog.addListWarning( ConflictLog.DELETE, "Modifications in " + "deleted subtree.", bn, mlistA.getEntry( ix ).getNode(), null );
    			...
    			if ( op2 == DELETE && mlistA.getEntry( matchIx ).locked ) {
    				clog.addListWarning( ConflictLog.DELETE, "Moved or copied node " + "deleted. Moving on by allowing the delete.", bn, mlistA.getEntry( ix ).getNode(), null );
//    				clog.addListConflict( ConflictLog.DELETE, "Moved or copied node " + "deleted. Moving on by allowing the delete.", bn, mlistA.getEntry( ix ).getNode(), null );
    			...
    			clog.addListConflict( ConflictLog.MOVE, "Node moved to different " + "locations - trying to recover by ignoring move inside childlist " + "(copies and inserts immediately following the node may have been " + "deleted)", bn, op1node, op1node.getFirstPartner( BranchNode.MATCH_FULL ) );
    			...
    			// CONFLICTCODE
    			clog.addListConflict( ConflictLog.MOVE, "Node moved and deleted - trying" + " to recover by deleting the node (copies and inserts immediately " + "following the node may also have been deleted)", bn, mlistA.getEntry( mlistA.matchInList( bn ) ).getNode(), null );
    			...
    			if ( isMovefMovefConflict( bn ) ) {
    				// CONFLICTCODE
    				clog.addListConflict( ConflictLog.MOVE, "The node was moved to " + "different locations. It will appear at each location.", bn, bn.getLeft().getFullMatch(), bn.getRight().getFullMatch() );
    				///System.out.println("CONFLICT: Node is far-moved to two different locations. " + "Implicit copies will occur in the output.");
    			}
    		}
    		else if ( op1 == MOVE_F && op2 == DELETE ) {
    			// CONFLICTCODE here
    			clog.addListConflict( ConflictLog.MOVE, "The node was moved and " + "deleted. Ignoring the deletion.", bn, bn.getLeft().getFullMatch(), bn.getRight().getFullMatch() );
