...
public class XMLPrinter extends DefaultHandler {
	...
	public void startDocument() {
		...
		pw.print( "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + ( prettyPrint ? Constants.LINE_SEPARATOR : "" ) );
		state			= STATE_TAG;
	}
	...
	public void startElement( String uri, String name, String qName, Attributes atts ) {
		...
		if( state == STATE_TAG && !prettyPrint)
			tagopen.append( Constants.LINE_SEPARATOR );
		tagopen.append('<');
	...
	public void endElement( String uri, String name, String qName ) {
		...
		if ( childcounter == null ) {
			...
			printWithNL( ( prettyPrint ? IND.substring( 0, indent ) : "" ) + "</" + qName + ">", prettyPrint );
		}
	...
	public void characters( char ch[], int startpos, int length ) {
		...
		int start=0,next=-1;
	   	do {
		   	next=chars.indexOf( Constants.LINE_SEPARATOR, start);
		   	if( next==-1)
