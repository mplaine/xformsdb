...
public class XFormsDBHandler extends BaseHandler {
	...
	private static final Logger logger								= Logger.getLogger( XFormsDBHandler.class );
		...
		if ( name != null && XFormsDBHandler.NAME_DTD.equals( name ) == true ) {
			...
			if ( publicId != null && Constants.DOCTYPE_XFORMSDB1_PUBLIC_ID.equals( publicId ) == true ) {
				try {
					...
					this.systemId			= null;
					//this.docType			= null;
					logger.log( Level.DEBUG, "A local copy of the XFormsDB 1.0 DTD file has been successfully retrieved based on the Public ID." );
				} catch ( Exception ex ) {
					// No action; just let the null InputSource pass through
					logger.log( Level.DEBUG, "Failed to retrieve a local copy of the XFormsDB 1.0 DTD file based on the Public ID." );
				}
			}
			// System ID for the XFormsDB 1.0 DTD file
				...
				try {
					...
					//this.docType			= null;
					logger.log( Level.DEBUG, "A local copy of the XFormsDB 1.0 DTD file has been successfully retrieved based on the System ID." );
				} catch ( Exception ex ) {
					// No action; just let the null InputSource pass through
					logger.log( Level.DEBUG, "Failed to retrieve a local copy of the XFormsDB 1.0 DTD file based on the System ID." );
				}
			}
		...
		// Parameter entity
			...
			if ( publicId == null && systemId.indexOf( Constants.DOCTYPE_XHTML_LAT1_SYSTEM_ID ) != -1 ) {
				try {
					...
					inputSource				= new InputSource( inputStream );
					logger.log( Level.DEBUG, "A local copy of the XHTML lat1 ENT file has been successfully retrieved based on the System ID." );
				} catch ( Exception ex ) {
					// No action; just let the null InputSource pass through
					logger.log( Level.DEBUG, "Failed to retrieve a local copy of the XHTML lat1 ENT file based on the System ID." );
				}
			}
				...
				try {
					...
					inputSource				= new InputSource( inputStream );
					logger.log( Level.DEBUG, "A local copy of the XHTML special ENT file has been successfully retrieved based on the System ID." );
				} catch ( Exception ex ) {
					// No action; just let the null InputSource pass through
					logger.log( Level.DEBUG, "Failed to retrieve a local copy of the XHTML special ENT file based on the System ID." );
				}
			}
				...
				try {
					...
					inputSource				= new InputSource( inputStream );
					logger.log( Level.DEBUG, "A local copy of the XHTML symbol ENT file has been successfully retrieved based on the System ID." );
				} catch ( Exception ex ) {
					// No action; just let the null InputSource pass through
					logger.log( Level.DEBUG, "Failed to retrieve a local copy of the XHTML symbol ENT file based on the System ID." );
				}
			}
		}
		// General entity
